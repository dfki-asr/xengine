...
struct INBwdContext {
  ...
  INBwdContext()
      : in_diff_mem(nullptr), src_mem(nullptr), gamma_beta_mem(nullptr),
        gamma_mem(nullptr), beta_mem(nullptr), mean_mem(nullptr),
        var_mem(nullptr), weights_mem(nullptr), gamma_beta_diff_mem(nullptr),
        out_diff_mem(nullptr), bwd_desc(nullptr), bwd_pd(nullptr),
        instancenorm_bwd(nullptr) {}
...
public:
  ...
  prepare_gamma_beta(memory::desc &gamma_beta_md,
    ...
    if (cpu_eng.get_kind() != engine::kind::cpu ||
        tensors[beta_name]->get_memory().get_engine().get_kind() !=
            engine::kind::cpu) {
      throw runtime_error(
          "InstanceNormalization: gamma and beta must be on CPU!");
    }
  ...
  void backward(shared_ptr<Device> dev,
    ...
    auto gamma_beta_diff_name = _b_op.output.at(1);
    string mean_name, var_name;
    if (_b_op.input.size() > 4) {
      mean_name = _b_op.input.at(4);
    } else {
      mean_name = name + "_mean";
    }
    if (_b_op.input.size() > 5) {
      var_name = _b_op.input.at(5);
    } else {
      var_name = name + "_var";
    }
    auto src_md = tensors[src_name]->desc();
    ...
    if (_bwd_context == nullptr) {
      auto time_create = get_time();
      _bwd_context.reset(new INBwdContext());
      ...
      _bwd_context->instancenorm_bwd.reset(
    ...
    auto time_exe = get_time();
    _bwd_context->instancenorm_bwd->execute(s, args);
    s.wait();
    if (measure_time) {
      timings[time_name]["exe"] = get_elapsed_ms(time_exe);
      timings[time_name]["total"] = get_elapsed_ms(begin);
